...
\ecjHeader{x}{x}{xxx-xxx}{201X}{Learning to Generate Genotypes with Neural Networks}{A.W. Churchill, S. Sigtia and C. Fernando}
...
% Typically, an EDA consists of a generative probabilistic model or some other unsupervised learning technique to learn the structure and dependencies of the good solutions at every generation. The way we propose to use the autoencoder deviates slightly from the standard EDA. We do not use the autoencoder to learn a probability distribution over good solutions, like in an EDA. Instead we use the autoencoder to directly learn a mapping from 

% The structure can be learnt either by using generative models (many many examples), or techniques that create a compressed representation like the generative grammar (Watson). 

% Although neural networks are normally seen as supervised methods (classification, regression), much of their success is due to their ability to learn features in an unsupervsied way. 
...
\caption{Evolutionary optimisation with the NADE or denoising autoencoder models }          % give the algorithm a caption
...
\begin{algorithmic}                    % enter the algorithmic environment
    ...
    \STATE $NICHING \Leftarrow \text{[True \OR False]}$\COMMENT{Use niching?}
    ...
    \STATE $model \Leftarrow \text{dA \OR NADE}$ \COMMENT{Choose neural network model}
    \STATE $pop \Leftarrow \text{Initialise population}$
    \WHILE{$evalsCount < EVALS$}
...
\begin{figure}[t!]
\centering
\subfigure[Showing how a bit string is partitioned on the Royal Road with Linkages]{
\makebox[1.0\textwidth]{
        \includegraphics[scale=0.7]{churchill_road_example1.png}
    }
}
\subfigure[An example optimal solution on the Royal Road with Linkages]{
\makebox[1.0\textwidth]{
        \includegraphics[scale=0.7]{churchill_road_example2.png}
    }
}
\caption[Optional caption for list of figures]{Showing samples from the dA and NADE on the Royal Road with Linkages problem.}
...
\makebox[1.0\textwidth]{
  ...
  \caption{Showing statistics from samples from the dA and NADE models on the Weing8 Knapsack instance.}
